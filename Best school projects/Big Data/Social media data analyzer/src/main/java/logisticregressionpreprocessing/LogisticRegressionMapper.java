...
public class LogisticRegressionMapper extends Mapper<LongWritable, Text, Text, Text> {
    ...
    public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
        ...
        // region --- sentiment extraction ---
        String sentiment = parts[parts.length - 3].trim();
        // endregion
        ...
        //
        // System.out.println("Keyword: " + sentiment + " " + keyword);
        // }
        // }
        ...
        if (keywords != null) {
            ...
            for (int i = 0; i < keywords.length; i++) {
                keywords[i] = keywords[i].trim().replaceAll("[^a-zA-Z]", "");

            }
            context.write(new Text("MapperKey" + key.toString()),
                    new Text(mainEmotion + " " + java.util.Arrays.toString(keywords)));
            System.out.println("Mapper: " + new Text(java.util.Arrays.toString(keywords)));
        }
        // endregion
